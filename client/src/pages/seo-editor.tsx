          ...
          {phase === "setup" && (
            <ScrollArea className="flex-1">
              <div className="px-5 py-4 space-y-3">
                ...
                <div>
                  <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block mb-1.5">Ниша / тема сайта</label>
                  <input data-testid="input-niche" value={niche} onChange={e => setNiche(e.target.value)} placeholder="напр. нейросети для бизнеса" className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 outline-none focus:border-indigo-400" />
                </div>
                <div>
                  <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block mb-1.5">Реф-ссылка по умолчанию</label>
                  <input value={targetUrl} onChange={e => setTargetUrl(e.target.value)} placeholder="https://ваш-сервис.ru" className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 outline-none focus:border-indigo-400" />
                </div>
                <div>
                  <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block mb-1.5">Текст кнопки CTA</label>
                  <input value={ctaLabel} onChange={e => setCtaLabel(e.target.value)} placeholder="Попробовать → / Играть → / Перейти →" className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 outline-none focus:border-indigo-400" />
                </div>
                <div>
              ...
              {showPages && (
                <>
                  <div className="px-4 py-3 border-b border-slate-100 flex gap-2">
                    ...
                    <button title="Изменить ключевые слова" onClick={() => { setPhase("setup"); setKeywordsText(cfg.rawKeywords.join("\n")); setNiche(cfg.niche); }} className="px-2.5 rounded-xl border border-slate-200 text-slate-400 hover:text-slate-700 hover:bg-slate-50">
