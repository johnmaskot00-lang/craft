...
7. Не удаляй canonical, Open Graph, JSON-LD, FAQ, breadcrumbs, внутренние ссылки и логотип. Не добавляй «Короткий ответ» / key-takeaways и «Содержание» / TOC.
8. На главной сохрани ровно один data-seo-article-feed и .article-card только внутри него (без дублей снаружи). Hero — один из 7 интерактивных вариантов. Пагинация только клиентская (кнопки ← / →) — НЕ создавай /page/2/ и отдельные HTML страниц пагинации.
9. В статьях запрещены SVG-анимации. Живость — фото, callout, pull-quote. До 3 фото. Если у сайта есть оффер владельца — в каждой статье две нативные рекомендации (класс offer-native), текст пишешь сам под тему статьи, со ссылкой на URL оффера. Не спамь главную. Не CTA на конкурентов. «Читайте также» — только реальные URL сайта.
